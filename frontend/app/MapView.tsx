...
export default function MapView({places}:{places:Place[]}){
 ...
 useEffect(()=>{let alive=true;(async()=>{const L=await import("leaflet");if(!alive||!el.current)return;if(map.current){map.current.remove();map.current=null;}const m=L.map(el.current,{zoomControl:true,attributionControl:true}).setView([37.4563,126.7052],11);map.current=m;L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap"}).addTo(m);const bounds:L.LatLngExpression[]=[];places.forEach(p=>{bounds.push([p.latitude,p.longitude]);const icon=L.divIcon({className:"place-marker",html:"<span>⌖</span>",iconSize:[38,38],iconAnchor:[19,19]});const detail=[p.category,p.address,p.phone].filter(Boolean).join("<br>");L.marker([p.latitude,p.longitude],{icon}).addTo(m).bindPopup(`<b>${p.name}</b><br>${detail}`)});if(bounds.length>1)m.fitBounds(bounds as L.LatLngBoundsExpression,{padding:[32,32]});else if(bounds.length===1)m.setView(bounds[0],13);setTimeout(()=>m.invalidateSize(),50)})();return()=>{alive=false;if(map.current){map.current.remove();map.current=null}}},[places]);
